...
def deep_map(f, s):
    ...
    True
    """
    for i in range(len(s)): # mind 'range' gaps!
        if type(s[i]) == list:
            deep_map(f,s[i])
        else:
            s[i] = f(s[i])
    ...
    #         deep_map(f,s[i])
    #     s[i] = f(s[i])
#  There is no 'return', don't stay lazy to forget 'else'!
...
def balanced(m):
    ...
    F1**L1 == F2*L2
    """
    def arm_lenth (m, side):
        assert side == 'left' or side == 'right'
        if side == 'left':
            return length(left(m))
        else:
            return length(right(m))
    def arm_mass (m, side):
        assert side == 'left' or side == 'right'
        if side == 'left':
            return total_mass(end(left(m)))
        else:
            return total_mass(end(right(m)))
    def moment(m, side):
        assert side == 'left' or side == 'right'
        if side == 'left':
            return arm_lenth(m, 'left') * arm_mass(m, 'left')
        else:
            return arm_lenth(m, 'right') * arm_mass(m, 'right')
    def is_balanced(m):
        return moment(m, 'left') == moment(m, 'right')
    if is_planet(m):
        return True
    else:
        assert is_mobile(m), "must get total mass of a mobile or a planet"
        return is_balanced(m) and balanced(end(left(m))) and balanced(end(right(m)))
