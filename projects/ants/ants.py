...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if  other is not self and other.is_container is False and self.ant_contained is None:
            return True
        return False
        # END Problem 8a
    ...
    def store_ant(self, ant):
        ...
        if self.can_contain(ant):
            ...
            # ant.place = self
            # do not assert the same place in the function.:w
        else:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        if self.place.bees:
            for bee in self.place.bees[:]: # attention: when you remove contents while iterating, use [:]
                bee.reduce_health(self.damage)
# END Problem 9
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower can throw leaves in water."""
    name = 'Scuba'
    food_cost = 6
    implemented = True
    is_waterproof = True
    ...
    def action(self, gamestate):
        ...
        def inspiration(next_place):
            ...
            if next_place.ant:
                ...
                next_ant.double()
                # next_ant.damage *= 2
                if next_ant.is_container and next_ant.ant_contained:
                    next_ant.ant_contained.double()
                    # next_ant.ant_contained.damage *= 2
            inspiration(next_place.exit)
        super().action(gamestate)
        inspiration(self.place.exit) 
        # END Problem 12
