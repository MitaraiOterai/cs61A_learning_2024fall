...
def distance(city_a, city_b):
    ...
    """
    def diff (x, y):
        assert type(x) == type(y) == int or float
        return abs(x - y)
    def l_diff(whitch_l, a, b):
        a = get_lat(city_a) if whitch_l == 'lat' else get_lon(city_a)
        b = get_lat(city_b) if whitch_l == 'lat' else get_lon(city_b)
        # a = city_a(get_lat) if whitch_l == 'lat' else city_a(get_lon)
        # b = city_b(get_lat) if whitch_l == 'lat' else city_b(get_lon)
        # print(a,b)
        return diff(a,b)
    def square(x):
        return pow(x, 2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    third_city = make_city('third_city', lat, lon)
    dist_a = distance(third_city, city_a)
    dist_b = distance(third_city, city_b)
    if dist_a < dist_b:
        return get_name(city_a)
    elif dist_a == dist_b:
        return 'In same distance.'
    else:
